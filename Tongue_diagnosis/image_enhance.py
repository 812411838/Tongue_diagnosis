...
def TestOneDir():
    ...
    for a, b, c in os.walk(root_path):
        for file_i in c:
            ...
            # cv2.imwrite(os.path.join(save_path, file_i[:-10] + "_horizontal.png"), img_horizontal)


            # img_move = Move(img_i,100,0)
...
def Hor(save_path):
    ...
    for a, b, c in os.walk(save_path):
        for file_i in c:
            ...
            img_horizontal = Horizontal(img_i)
            cv2.imwrite(os.path.join(save_path, file_i[:-8] + "_horizontal.png"), img_horizontal)
    return 0
def Bri(save_path):
    for a, b, c in os.walk(save_path):
        for file_i in c:
            ...
            img_brighter = cv2.convertScaleAbs(img_i, alpha=1.2, beta=0)
            cv2.imwrite(os.path.join(save_path, file_i[:-4] + "_brighter.png"), img_brighter)

            img_darker = cv2.convertScaleAbs(img_i, alpha=0.8, beta=0)
...
def salt(save_path):
    ...
    for a, b, c in os.walk(root_path):
        for file_i in c:
            ...
            cv2.imwrite(os.path.join(save_path, file_i[:-4] + ".png"), img_i)
            img_salt = SaltAndPepper(img_i, 0.05)

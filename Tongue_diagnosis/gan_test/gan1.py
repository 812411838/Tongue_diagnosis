...
class Config():
    ...
    d_net_path = 'snapshots256/dnet.pth' #判别网络权重文件保存的路径
    g_net_path = 'snapshots256/gnet.pth' #生成网络权重文件保存的路径
...
if not os.path.exists('snapshots256'):
    os.mkdir('snapshots256') #生成snapshots文件夹
    ...
    def __init__(self, opt):
        ...
        self.generate = nn.Sequential(
           ...
           #input = (n, c, h, w = 256, 100, 1, 1)
            ...
            # decon = (input - 1)*stride + k - 2*padding = (4-1)*2 + 4-2 = 8 16
            ...
            # decon = (input - 1)*stride + k - 2*padding = (8-1)*2 + 4-2 = 16 64
            ...
            # decon = (input - 1)*stride + k - 2*padding = (16-1)*2 + 4-2 = 32 128
...
class Dnet(nn.Module):
    def __init__(self, opt):
        ...
        self.discrim = nn.Sequential(
            ...
            nn.Conv2d(in_channels= self.feats * 2, out_channels= self.feats * 4, kernel_size= 4, stride= 4, padding= 0,bias=False),
            nn.BatchNorm2d(self.feats * 4),
            nn.LeakyReLU(0.2, True),


            nn.Conv2d(in_channels= self.feats * 4, out_channels= self.feats * 8, kernel_size= 4, stride= 4, padding= 0, bias=False),
            nn.BatchNorm2d(self.feats *8),
            nn.LeakyReLU(0.2, True),
...
for epoch in range(opt.max_epoch):  #总循环轮次
    ...
    if epoch % 5 == 0:
        torch.save(d_net.state_dict(), "./weights256/d_net_model-{}.pth".format(epoch))
        torch.save(g_net.state_dict(), "./weights256/g_net_model-{}.pth".format(epoch))
